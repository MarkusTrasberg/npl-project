...
class CustomRetriever(BaseRetriever):
    ...
    def retrieve(self):
        ...
        test_corpus = [word_tokenize(data) for data in self.dataset_reader.generate_input_field_corpus(self.test_ds)]
        for idx in trange(len(self.test_ds), disable=not self.is_main_process):
            query = self.test_ds[idx]
            print(f'non tokenized query: {query}')

            print(f'non tokenized query: {test_corpus}')
