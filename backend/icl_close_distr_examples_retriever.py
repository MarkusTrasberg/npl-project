...
class CloseDistrExamplesRetriever(BaseRetriever):
    """Finds in-context examples that represent the distribution of the whole dataset.
    ...
    def __init__(self,
        ...
        # self.rtr_idx_list = self.find_k_examples(self.ice_num)
    ...
    def knn_search(self, ice_num):
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            print('near_ids', near_ids)
            rtr_idx_list[idx] = near_ids
        
        print('rtr_idx_list', rtr_idx_list)
        return rtr_idx_list
    

    def find_k_examples(self, ice_num):
        res_list = self.forward(self.dataloader, process_bar=True, information="Embedding test set...")
        rtr_idx_list = [[] for _ in range(len(res_list))]
        logger.info("Retrieving data for test set...")
        
        # Step 1: Calculate pairwise cosine similarity
        similarity_matrix = cosine_similarity(self.embed_list)
        print(similarity_matrix.shape)
        # Step 2: Compute sum of cosine similarities for each sentence with all other sentences
        similarity_sums = np.sum(similarity_matrix, axis=1)
        
        # Step 3: Select the first sentence with the highest sum of cosine similarities
        representative_idx = np.argmax(similarity_sums)
        representative_ids = [int(representative_idx)]
        print('represantive id',representative_idx)
        print('represantive sentence',self.select_datalist[representative_idx])

        # Steps 4 and 5: Find the second and subsequent representative sentences
        for i in range(1, ice_num):
            similarity_sums[representative_idx] = -np.inf  # Set previously selected representative's similarity to -inf
            representative_idx = np.argmax(similarity_sums)
            representative_ids.append(int(representative_idx))
            print('represantive id',representative_idx)
            print('represantive sentence',self.select_datalist[representative_idx])

        print('representative_ids',representative_ids)	

        # Step 6: add global representative examples for each test example   
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            idx = entry['metadata']['id']
            rtr_idx_list[idx] = representative_ids
    ...
    def retrieve(self):
        return self.find_k_examples(self.ice_num)
